...
class TgPostsMediaDataRepository:
    ...
    async def add_one(cls, payload: AddTgPostsMediaData) -> Optional[TgPostsMediaDataModel]:
        description_usage_json = json.dumps(
            payload.description_usage) if payload.description_usage is not None else None
        tag_usage_json = json.dumps(
            payload.tag_usage) if payload.tag_usage is not None else None
        structured_description_usage_json = json.dumps(
            payload.structured_description_usage) if payload.structured_description_usage is not None else None
    ...
    @classmethod
    async def get_all_by_media_id(cls, media_id: UUID) -> list[TgPostsMediaDataModel]:
        async with await psycopg.AsyncConnection.connect(
            user=DB_USER,
            password=DB_PASSWORD,
            host=DB_HOST,
            ...
            async with aconn.cursor(row_factory=class_row(TgPostsMediaDataModel)) as acur:
                await acur.execute(sql.SQL("SELECT * FROM media_datas WHERE media_id = %s"), (media_id,))
                tg_post_media_data_items = await acur.fetchall()
                return tg_post_media_data_items
